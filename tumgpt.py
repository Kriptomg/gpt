...
def format_btc_whale_report(all_period_data, all_xchain_data, gunluk_hacim, gunluk_fiyat, hacim_var, hacim_error, now_tr):
    out = [f"\n━━ 🐋 Balina Transfer Analizi ━━"]
    out.append(f"Tarih/Saat (TSI): {now_tr}\n")
    if not hacim_var:
        out.append(f"⚠️ CoinGecko veri eksik: {hacim_error}. Bu coin için oran ve öneri gösterilemiyor.")
    son_data = all_period_data[-1][1]
    fark = son_data["in_amount"] - son_data["out_amount"]
    yon = get_period_yon(son_data)
    genel_yorum = yorum_uret(fark * gunluk_fiyat if gunluk_fiyat else fark, gunluk_hacim, yon, hacim_var)
    ...
    for i, (label, data) in enumerate(all_period_data):
        ...
        oran = (abs(fark_usd) / gunluk_hacim * 100) if (gunluk_hacim and hacim_var) else 0
        oran_s = f"{oran:.2f}%"
        out.append(
            f"\n{label}\n"
...
def btc_kisavadeli_analizler(ohlcv_dict, current_price, dtstr_tr, dtstr_utc):
    ...
    for vade, ohlcv in vadeler:
        ...
        if vade == "5dk":
            results.append("⚠️ 5dk'lık analizlerde volatilite ve ATR genellikle düşüktür, ani hareketler yanıltıcı olabilir.")
...
def btc_teknik_analiz_raporu(
    ...
    obv_1h_pct = (100 * (obv_val - obv_prev) / abs(obv_prev)) if (obv_val is not None and obv_prev is not None and abs(obv_prev) > 0) else None
    ...
    if boll_ma is not None and boll_up is not None and boll_down is not None:
        rapor.append(f"• Bollinger: MA {boll_ma[-1]:.2f} | Üst {boll_up[-1]:.2f} | Alt {boll_down[-1]:.2f}")
    else:
    ...
    return "\n".join(rapor), score, max_score, destek, direnç, ema7, ema21, macd_line, rsi_val, obv_val, trend, obv_1h_pct
...
def btc_piyasa_analiz_turkce():
    ...
    for interval, label in intervals:
        ...
        if funding_rate is not None:
            lines.append(f"• Fonlama Oranı: {funding_rate:.5f} ({'Pozitif' if funding_rate > 0 else 'Negatif'})\n  (Vadeli işlem fonlama oranı. Negatif ise short pozisyonlar daha baskın.)")
            any_data = True
        if ratio is not None:
            lines.append(f"• Uzun/Kısa Oranı: {ratio:.2f} (1'in altı short ağırlık demektir.)")
            any_data = True
        if open_interest is not None:
            lines.append(f"• Açık Pozisyon: {open_interest:,.0f} BTC (Piyasadaki toplam açık kontrat miktarı.)")
            any_data = True
        ...
        if bids is not None and asks is not None:
            lines.append(f"• Emir Derinliği: Alış: {bids:.2f} BTC | Satış: {asks:.2f} BTC")
            any_data = True
        if any_data:
            out += "\n".join(lines) + "\n"
    return out

def nihai_oneri(skor_5m, skor_15m, skor_30m, skor_1h, skor_4h, skor_1d, trend_1h, trend_4h, trend_1d):
    karar = "TUT"
    simge = "🟡"
    gerekce = ""
    if skor_1h >= 4 and skor_4h >= 2 and skor_1d >= 2 and (skor_5m >= 1 or skor_15m >= 1):
        karar = "AL"
        simge = "🟢"
...
async def main_loop():
    ...
    while True:
        ...
        per_coin, per_coin_xchain = analyze_all_periods(all_messages, now_utc.replace(tzinfo=timezone.utc))
        gunluk_hacim, hacim_error = safe_api_call(get_daily_volume_usd, max_retry=2, wait=5, coin="BTC")
        gunluk_fiyat, _ = safe_api_call(get_daily_price, max_retry=2, wait=5, coin="BTC")

        # DÜZELTME 1: Balina net akışı dinamik hesaplanıyor (son 1 saat)
        ...
        # DÜZELTME 2: OBV etkisi teknik analizde artırıldı (yukarıda fonksiyonda)
        ...
        # DÜZELTME 3: balina_net_1h dinamik geldi, ls_ratio_1h testte olduğu gibi alınmış (istersen dinamik de yapılabilir)
        ...
        nihai = nihai_oneri(ks_5m, ks_15m, ks_30m, skor_1h, skor_4h, skor_1d, trend, trend_4h, trend_d)
        ...
        for coin in COINGECKO_IDS:
            gunluk_hacim, _ = safe_api_call(get_daily_volume_usd, max_retry=2, wait=5, coin=coin)
            gunluk_fiyat, _ = safe_api_call(get_daily_price, max_retry=2, wait=5, coin=coin)
